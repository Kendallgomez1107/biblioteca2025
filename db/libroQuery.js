...
/**
 * Carga la lista de libros (en este ejemplo solo 10 libros)
 */
...
const crearLibroQuery = async (libro) => {
    const { nombre, codigo, precio, disponible } = libro;
    return new Promise((resolve, reject) => {
        const sql = 'INSERT INTO libros (nombre, codigo, precio, disponible) VALUES (?, ?, ?, ?)';
...
const actualizarLibroQuery = (id, libro) => {
    const { nombre, codigo, precio, disponible } = libro;
    return new Promise((resolve, reject) => {
        const sql = 'UPDATE libros SET nombre = ?, codigo = ?, precio = ?, disponible = ? WHERE id = ?';
